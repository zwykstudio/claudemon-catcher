...
if command -v claude &>/dev/null; then
    ...
    echo "  Command: claude config set statusline \"python3 $STATUSLINE_SCRIPT\""
    echo ""
    read -rp "  Install statusline? [Y/n] " SL_CONFIRM
    SL_CONFIRM="${SL_CONFIRM:-Y}"
    if [[ "$SL_CONFIRM" =~ ^[Yy] ]]; then
        claude config set statusline "python3 $STATUSLINE_SCRIPT" 2>/dev/null && \
            echo "  ✓ Statusline configured" || \
            echo "  ⚠ Failed — run manually: claude config set statusline \"python3 $STATUSLINE_SCRIPT\""
    else
        echo "  ⚠ Skipped. To add later:"
        echo "    claude config set statusline \"python3 $STATUSLINE_SCRIPT\""
    fi
else
